...
export class AuthService {
  ...
  createUser(email: string, password: string) {
    ...
    this.http.post("http://localhost:3005/auth/signup", authData)
      .subscribe(response => {
  ...
  login(email: string, password: string) {
    const authData: AuthData = { email: email, password: password }
    this.http.post<{token: string, expiresIn: number, userId: string}>("http://localhost:3005/auth/login", authData)
      .subscribe(response => {
